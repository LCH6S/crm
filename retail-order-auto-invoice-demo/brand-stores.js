...
function activateBrandDetailTab(panelId) {
  ...
  if (panelId === "brandProductManagementPanel") {
    const brand = getCurrentBrand();
    if (brand) renderBrandProductFeatures(brand, getBrandCustomer(brand));
  }
}
...
function renderBrandStoreDetail() {
  ...
  const fields = [
    { label: "门店名", value: store.name },
    { label: "门店号", value: store.storeNo },
    { label: "门店类型", value: store.type },
    {
      label: "门店状态",
      html: `<button class="brand-store-status-switch ${store.enabled ? "enabled" : "disabled"}" id="brandStoreStatusToggle" type="button" role="switch" aria-checked="${store.enabled}"><span>${store.enabled ? "启用" : "禁用"}</span><i aria-hidden="true"></i></button>`,
    },
    { label: "所属地区", value: store.region },
    { label: "详细地址", value: store.address },
    { label: "联系电话", value: store.phone || "-" },
    { label: "备注", value: store.remark || "-" },
    { label: "门店 ID", value: store.id },
    { label: "创建时间", value: store.createdAt },
  ];
  document.getElementById("brandStoreDetailGrid").innerHTML = fields.map((field) => `<div><dt>${field.label}</dt><dd>${field.html || brandStoreSafe(field.value)}</dd></div>`).join("");
}

function toggleBrandStoreStatus() {
  const store = getCurrentBrandStore();
  if (!store) return;
  store.enabled = !store.enabled;
  renderBrandStoreDetail();
  renderBrandStores();
  showToast(`门店已${store.enabled ? "启用" : "禁用"}`);
}
...
function bindBrandStoreEvents() {
  ...
  document.body.addEventListener("click", (event) => {
    const statusToggle = event.target.closest("#brandStoreStatusToggle");
    if (statusToggle) {
      toggleBrandStoreStatus();
      return;
    }
