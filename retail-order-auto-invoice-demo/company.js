const companyState = {
  ...
  type: "",
  invoiceStatus: "",
  managedFilters: { name: "", uscc: "", customerName: "", customerNumber: "", type: "", invoiceStatus: "" },
...
companies.forEach((company) => {
  company.customerId ||= "160247730638";
  company.registrationCountry ||= "CN";
});
...
const batchTasks = [
  {
    id: "TASK-20260720-006",
    type: "CREATE_COMPANY",
    fileName: "模拟_创建公司_执行失败.xlsx",
    operator: "刘辰浩",
    createdAt: "2026-07-20 18:02",
    ...
    skipped: 139,
    rows: [
      { row: 1, uscc: "91110000100010433L", name: "中国石油天然气集团有限公司", check: "通过", execute: "失败", reason: "任务执行异常，请稍后重试" },
      { row: 2, uscc: "91310115MA1FAIL001", name: "华东示例商贸有限公司", check: "不通过", execute: "跳过", reason: "公司信息检查未通过" },
    ],
  },
  {
    id: "TASK-20260720-005",
    type: "CREATE_COMPANY",
    fileName: "模拟_创建公司_检查中.xlsx",
    operator: "刘辰浩",
    createdAt: "2026-07-20 17:11",
    status: "Checking",
    total: 155,
    pass: 0,
    fail: 0,
    ...
    skipped: 0,
    rows: [
      { row: 1, uscc: "91110000100010433L", name: "中国石油天然气集团有限公司", check: "检查中", execute: "-", reason: "-" },
      { row: 2, uscc: "913100001322004345", name: "上海烟草集团有限责任公司", check: "待检查", execute: "-", reason: "-" },
    ],
  },
  {
    id: "TASK-20260720-004",
    type: "CREATE_COMPANY",
    fileName: "模拟_创建公司_检查失败.xlsx",
    operator: "刘辰浩",
    createdAt: "2026-07-20 16:36",
    status: "CheckFailed",
    total: 155,
    pass: 0,
    fail: 155,
    ...
    skipped: 0,
    rows: [
      { row: 1, uscc: "-", name: "-", check: "不通过", execute: "-", reason: "文件格式不正确，请使用最新模板" },
    ],
  },
  {
    id: "TASK-20260720-003",
    type: "ENABLE_INVOICE",
    fileName: "模拟_开通发票_执行中.xlsx",
    operator: "刘辰浩",
    createdAt: "2026-07-20 16:08",
    status: "Executing",
    total: 155,
    pass: 139,
    fail: 16,
    ...
    rows: [
      ...
      { row: 2, uscc: "913100001322004345", name: "上海烟草集团有限责任公司", taxpayerType: "一般纳税人", taxMethod: "简易计税", levyRate: "3%", check: "通过", execute: "执行中", reason: "-" },
      { row: 3, uscc: "91310115MA1FAIL001", name: "华东示例商贸有限公司", taxpayerType: "-", taxMethod: "-", levyRate: "-", check: "不通过", execute: "跳过", reason: "公司信息检查未通过" },
    ],
  },
  {
    id: "TASK-20260720-001",
    type: "ENABLE_INVOICE",
    fileName: "模拟_开通发票_待执行.xlsx",
    operator: "刘辰浩",
    createdAt: "2026-07-20 14:58",
    ...
    skipped: 0,
    rows: [
      { row: 1, uscc: "91110000100010433L", name: "中国石油天然气集团有限公司", taxpayerType: "一般纳税人", taxMethod: "一般计税", levyRate: "-", check: "通过", execute: "-", reason: "-" },
      { row: 2, uscc: "91310115MA1FAIL001", name: "华东示例商贸有限公司", taxpayerType: "-", taxMethod: "-", levyRate: "-", check: "不通过", execute: "-", reason: "公司信息检查未通过" },
    ],
  },
  {
...
// Existing sample tasks belong to this customer; new tasks record their actual owner.
batchTasks.forEach((task) => { task.customerId = "160247730638"; });
...
const companyCountryNames = { CN: "中国", MY: "马来西亚" };

function getCompanyCustomer(company) {
  return customers.find((customer) => customer.id === company.customerId) || null;
}

function findCompanyByUsccInCustomer(uscc, customerId = state.currentCustomerId) {
  return companies.find((company) => company.customerId === customerId && company.uscc === uscc) || null;
}

function renderCompanies() {
  const nameKeyword = companyState.nameKeyword.toLowerCase();
  const usccKeyword = companyState.usccKeyword.toLowerCase();
  const rows = companies.filter((company) => {
    if (company.customerId !== state.currentCustomerId) return false;
    const matchesName = !nameKeyword || company.name.toLowerCase().includes(nameKeyword);
    const matchesUscc = !usccKeyword || company.uscc.toLowerCase().includes(usccKeyword);
    return matchesName && matchesUscc && (!companyState.type || company.type === companyState.type) && (!companyState.invoiceStatus || company.invoiceStatus === companyState.invoiceStatus);
  });
  ...
  document.getElementById("companyRows").innerHTML = rows.length
    ? rows.map((company) => {
      ...
      return `<tr>
        ...
        <td>${companySafe(company.parent || "-")}</td><td>${companySafe(company.companyCode || company.id)}</td><td><span class="company-invoice-status ${status.className}">${status.label}</span></td>
        <td><div class="company-row-actions"><button class="link-btn" data-company-detail="${company.id}" data-company-detail-source="customer">详情</button><span class="separator">|</span><button class="link-btn" data-company-invoice-action="enable" data-company-id="${company.id}">开通</button></div></td>
      </tr>`;
    }).join("")
    : '<tr><td colspan="7" class="empty-cell">未找到匹配的公司</td></tr>';
}

function filteredManagedCompanies() {
  const filters = companyState.managedFilters;
  const nameKeyword = filters.name.toLowerCase();
  const usccKeyword = filters.uscc.toLowerCase();
  const customerNameKeyword = filters.customerName.toLowerCase();
  const customerNumberKeyword = filters.customerNumber.toLowerCase();
  return companies.filter((company) => {
    const customer = getCompanyCustomer(company);
    ...
    if (customerNameKeyword && ![customer.name, customer.shortName].some((value) => String(value || "").toLowerCase().includes(customerNameKeyword))) return false;
    if (customerNumberKeyword && !customer.id.toLowerCase().includes(customerNumberKeyword)) return false;
    if (filters.type && company.type !== filters.type) return false;
    if (filters.invoiceStatus && company.invoiceStatus !== filters.invoiceStatus) return false;
    return true;
  });
}

function renderManagedCompanies() {
  const rows = filteredManagedCompanies();
  document.getElementById("managedCompanyRows").innerHTML = rows.length ? rows.map((company) => {
    const customer = getCompanyCustomer(company);
    const status = companyStatusMeta(company.invoiceStatus);
    return `<tr>
      <td title="${companySafe(company.name)}">${companySafe(company.name)}</td>
      <td>${companySafe(company.uscc)}</td>
      <td><div class="managed-company-customer"><strong>${companySafe(customer.name)}</strong><span>${companySafe(customer.id)}</span></div></td>
      <td>${companySafe(companyCountryNames[company.registrationCountry] || "-")}</td>
      <td><span class="company-type ${company.type === "Head" ? "head-office" : "branch"}">${company.type === "Head" ? "总公司" : "分公司"}</span></td>
      <td title="${companySafe(company.parent || "-")}">${companySafe(company.parent || "-")}</td>
      <td>${companySafe(company.companyCode || company.id)}</td>
      <td><span class="company-invoice-status ${status.className}">${status.label}</span></td>
      <td><div class="company-row-actions"><button class="link-btn" type="button" data-company-detail="${companySafe(company.id)}" data-company-detail-source="management">详情</button><span class="separator">|</span><button class="link-btn" type="button" data-company-invoice-action="enable" data-company-id="${companySafe(company.id)}">开通</button></div></td>
    </tr>`;
  }).join("") : '<tr><td colspan="9"><div class="table-empty">暂无符合条件的公司</div></td></tr>';
  document.getElementById("managedCompanyTotal").textContent = `共${rows.length}条`;
}

function openCompanyManagement() {
  setActiveSidebar("companyManagementMenuBtn");
  renderManagedCompanies();
  setView("companyManagementView");
}
...
function renderCompanyDetail() {
  ...
  document.getElementById("companyBasicCustomer").textContent = customer ? `${customer.name}（${customer.id}）` : "-";
  document.getElementById("companyBasicRegistrationCountry").textContent = companyCountryNames[company.registrationCountry] || "-";
...
function openCompanyDetail(companyId, source = "customer") {
  const company = companies.find((item) => item.id === companyId);
  if (!company) return;
  const customer = getCompanyCustomer(company);
  companyState.detailSource = source;
  ...
  document.getElementById("companyDetailBreadcrumb").innerHTML = source === "management"
    ? "主页&nbsp;&nbsp;/&nbsp;&nbsp;公司管理&nbsp;&nbsp;/&nbsp;&nbsp;公司详情"
    : "主页&nbsp;&nbsp;/&nbsp;&nbsp;我的客户&nbsp;&nbsp;/&nbsp;&nbsp;客户详情&nbsp;&nbsp;/&nbsp;&nbsp;公司详情";
  document.getElementById("backFromCompanyDetailBtn").textContent = source === "management" ? "返回公司管理" : "返回客户详情";
  setActiveSidebar(source === "management" ? "companyManagementMenuBtn" : "myCustomersMenuBtn");
  renderCompanyDetail();
  setView("companyDetailView");
}

function returnFromCompanyDetail() {
  const managementSource = companyState.detailSource === "management";
  setActiveSidebar(managementSource ? "companyManagementMenuBtn" : "myCustomersMenuBtn");
  if (managementSource) renderManagedCompanies();
  setView(managementSource ? "companyManagementView" : "productsView");
}
...
function resetCreateCompanyForm() {
  ...
  document.getElementById("newCompanyParent").innerHTML = companies.filter((company) => company.customerId === state.currentCustomerId && company.type === "Head").map((company) => `<option value="${company.id}">${companySafe(company.name)}</option>`).join("");
}
...
function lookupCompanyName() {
  ...
  const existing = findCompanyByUsccInCustomer(input.value);
  if (existing) {
    error.textContent = "该统一社会信用代码在当前集团下已存在，请勿重复创建";
    nameInput.value = existing.name;
    return;
  }
  error.textContent = "";
  const registeredCompany = companies.find((company) => company.uscc === input.value);
  nameInput.value = registeredCompany?.name || `新建示例企业 ${input.value.slice(-4)} 有限公司`;
}
...
function setEnableInvoiceStep(step) {
  if (companyState.enableBlocked) step = 1;
...
function openEnableInvoice(companyId) {
  ...
  const customerProductBlocked = !getCompanyCustomer(company)?.einvoiceEnabled;
  const isReadonly = customerProductBlocked || company.invoiceStatus === "Opening" || company.invoiceStatus === "Opened";
  companyState.enableBlocked = customerProductBlocked;
  const drawer = document.getElementById("enableInvoiceDrawer");
  const customerProductTip = document.getElementById("enableInvoiceCustomerProductTip");
  drawer.classList.toggle("customer-product-disabled", customerProductBlocked);
  customerProductTip.classList.toggle("hidden", !customerProductBlocked);
  ...
  functionOption.setAttribute("aria-disabled", String(isReadonly));
  functionCheckbox.checked = !customerProductBlocked && company.invoiceStatus !== "Opened";
  ...
  functionDescription.textContent = customerProductBlocked
    ? "请先在集团维度开启电子发票功能"
    : (descriptions[company.invoiceStatus] || descriptions.Unopened);
...
function submitEnableInvoice() {
  ...
  if (!company) return;
  if (!getCompanyCustomer(company)?.einvoiceEnabled) {
    companyState.enableBlocked = true;
    setEnableInvoiceStep(1);
    return;
  }
...
function ensureCompanyTaxpayer(company) {
  let taxpayer = taxNos.find((item) => item.taxNo === company.uscc);
  const taxpayerType = company.taxpayerTypeLabel || company.application?.taxpayerType || "一般纳税人";
  const taxMethod = company.application?.taxMethod || (taxpayerType === "小规模纳税人" ? "简易计税" : "一般计税");
  const levyRate = taxMethod === "一般计税" ? "-" : (company.application?.levyRate || "3%");
  if (!taxpayer) {
    ...
    taxpayer = {
      ...
      taxMethod,
      levyRate,
      region: company.registrationCountry === "CN" ? "上海市" : "-",
      address: "-",
      phone: application?.phone || "-",
      bankName: "-",
      bankAccount: "-",
      createdAt: company.createdAt || "-",
      invoiceChannel: company.invoiceChannel || "-",
      invoiceUsers: application ? [{
        name: application.name || "-",
        taxAccount: "-",
        email: application.email || "-",
        role: application.role || "-",
        loginMode: "-",
        loginAccount: application.phone || "-",
    ...
    taxpayer.enabled = true;
    taxpayer.invoiceChannel = company.invoiceChannel || taxpayer.invoiceChannel || "-";
  }
  renderTaxNos();
  return taxpayer;
}
...
function buildInvoiceBatchRow(row, input) {
  const company = findCompanyByUsccInCustomer(input.uscc);
  let check = "通过";
  let reason = "-";
  if (!company) {
    check = "不通过";
    reason = "当前集团客户下未找到该公司";
  } else if (!getCompanyCustomer(company)?.einvoiceEnabled) {
    check = "不通过";
    reason = "请先在集团维度开启电子发票功能";
...
function getMockBatchRows(mode) {
  if (mode === "create") {
    return [
      ...
      { row: 3, uscc: "91310120MA1HRP974K", name: "上海悦投贸易有限公司", type: "总公司", parent: "-", check: "不通过", reason: "统一社会信用代码在当前集团下已存在" },
    ];
  }
  if (mode === "store") {
    return [
...
function openBatch(mode) {
  ...
  companyState.batchCustomerId = state.currentCustomerId;
...
function executeBatch() {
  ...
  if (companyState.batchMode === "brandStoreCreate") {
    ...
    passedRows.forEach((item) => {
      if (findCompanyByUsccInCustomer(item.uscc)) return;
      const parent = companies.find((company) => company.customerId === state.currentCustomerId && company.name === item.parent);
      companies.push({ id: `company-${Date.now()}-${item.row}`, companyCode: `G-COMP-${String(companies.length + 1).padStart(3, "0")}`, name: item.name, uscc: item.uscc, customerId: state.currentCustomerId, registrationCountry: "CN", type: item.type === "分公司" ? "Branch" : "Head", parentId: parent?.id || "", parent: item.parent, invoiceStatus: "Unopened", createdAt: "2026-07-20 20:30", remark: "批量创建" });
    });
  ...
  const task = {
    id: `TASK-${Date.now()}`,
    customerId: companyState.batchCustomerId,
...
function batchTaskStatusMeta(status) {
  const map = {
    Checking: { label: "检查中", className: "status-opening" },
    CheckFailed: { label: "检查失败", className: "status-failed" },
    CheckCompleted: { label: "待执行", className: "status-pending" },
    Executing: { label: "执行中", className: "status-opening" },
...
function batchTaskExecutionResult(task) {
  if (task.status !== "ExecutionCompleted") return '<span class="batch-execution-empty">-</span>';
  return `<span class="batch-execution-result"><span class="success-text">${task.success} 成功</span><span class="batch-execution-separator">/</span><span class="danger-text">${task.executeFail} 失败</span><span class="batch-execution-separator">/</span><span>${task.skipped} 跳过</span></span>`;
}
...
function renderBatchRecords() {
  ...
  const tasks = batchTasks.filter((task) => task.customerId === state.currentCustomerId && (!type || task.type === type) && (!status || task.status === status));
  document.getElementById("batchRecordRows").innerHTML = tasks.length ? tasks.map((task) => {
    ...
    return `<tr><td>${task.createdAt}</td><td>${batchTaskTypeLabel(task.type)}</td><td title="${companySafe(task.fileName)}">${companySafe(task.fileName)}</td><td>${task.operator}</td><td><span class="batch-status ${statusMeta.className}">${statusMeta.label}</span></td><td>${task.total}</td><td><span class="success-text">${task.pass}</span> / <span class="danger-text">${task.fail}</span></td><td>${batchTaskExecutionResult(task)}</td><td><button class="link-btn batch-detail-link" data-batch-task-detail="${task.id}">查看详情</button></td></tr>`;
  }).join("") : '<tr><td colspan="9" class="empty-cell">未找到匹配的批量操作记录</td></tr>';
}

function openBatchRecords() {
  const fromBatch = document.getElementById("companyBatchView").classList.contains("active");
  const customerId = fromBatch ? companyState.batchCustomerId : state.currentCustomerId;
  openCustomerDetail(customerId);
  activateCustomerTab("customerBatchRecordsPanel");
}
...
function openBatchTaskDetail(taskId) {
  const task = batchTasks.find((item) => item.id === taskId && item.customerId === state.currentCustomerId);
  if (!task) return;
  const statusMeta = batchTaskStatusMeta(task.status);
  document.getElementById("batchRecordDetailBreadcrumb").textContent = `主页 / 我的客户 / ${getCurrentCustomer().name} / 批量操作记录 / 任务详情`;
  document.getElementById("batchRecordDetailTitle").textContent = `${batchTaskTypeLabel(task.type)}任务详情`;
  document.getElementById("batchRecordDetailMeta").innerHTML = `${companySafe(task.id)} · ${companySafe(task.fileName)} · ${companySafe(task.operator)} · ${companySafe(task.createdAt)} · <span class="batch-status ${statusMeta.className}">${statusMeta.label}</span>`;
  const executionStats = task.status === "ExecutionCompleted"
    ? `<span class="success-text">执行成功：<strong>${task.success}</strong></span><span class="danger-text">执行失败：<strong>${task.executeFail}</strong></span><span>跳过：<strong>${task.skipped}</strong></span>`
    : "";
  document.getElementById("batchRecordDetailStats").innerHTML = `<span>总行数：<strong>${task.total}</strong></span><span class="success-text">通过：<strong>${task.pass}</strong></span><span class="danger-text">不通过：<strong>${task.fail}</strong></span>${executionStats}`;
...
function bindCompanyEvents() {
  ...
  document.getElementById("companyManagementMenuBtn").addEventListener("click", openCompanyManagement);
  document.getElementById("searchManagedCompaniesBtn").addEventListener("click", () => {
    companyState.managedFilters = {
      name: document.getElementById("managedCompanyNameKeyword").value.trim(),
      uscc: document.getElementById("managedCompanyUsccKeyword").value.trim(),
      customerName: document.getElementById("managedCompanyCustomerNameKeyword").value.trim(),
      customerNumber: document.getElementById("managedCompanyCustomerNumberKeyword").value.trim(),
      type: document.getElementById("managedCompanyTypeFilter").value,
      invoiceStatus: document.getElementById("managedCompanyInvoiceStatusFilter").value,
    };
    renderManagedCompanies();
  });
  ["managedCompanyNameKeyword", "managedCompanyUsccKeyword", "managedCompanyCustomerNameKeyword", "managedCompanyCustomerNumberKeyword"].forEach((id) => {
    document.getElementById(id).addEventListener("keydown", (event) => {
      if (event.key !== "Enter") return;
      event.preventDefault();
      document.getElementById("searchManagedCompaniesBtn").click();
    });
  });
  document.getElementById("resetManagedCompaniesBtn").addEventListener("click", () => {
    ["managedCompanyNameKeyword", "managedCompanyUsccKeyword", "managedCompanyCustomerNameKeyword", "managedCompanyCustomerNumberKeyword", "managedCompanyTypeFilter", "managedCompanyInvoiceStatusFilter"].forEach((id) => { document.getElementById(id).value = ""; });
    companyState.managedFilters = { name: "", uscc: "", customerName: "", customerNumber: "", type: "", invoiceStatus: "" };
    renderManagedCompanies();
  });
  ...
  document.getElementById("companySearchBtn").addEventListener("click", () => {
    companyState.nameKeyword = document.getElementById("companyNameKeyword").value.trim();
    companyState.usccKeyword = document.getElementById("companyUsccKeyword").value.trim();
  ...
  ["companyNameKeyword", "companyUsccKeyword"].forEach((id) => {
    document.getElementById(id).addEventListener("keydown", (event) => {
      if (event.key !== "Enter") return;
      event.preventDefault();
      document.getElementById("companySearchBtn").click();
    });
  });
  document.getElementById("companyResetBtn").addEventListener("click", () => {
    ["companyNameKeyword", "companyUsccKeyword", "companyTypeFilter", "companyInvoiceStatusFilter"].forEach((id) => { document.getElementById(id).value = ""; });
    companyState.nameKeyword = ""; companyState.usccKeyword = ""; companyState.type = ""; companyState.invoiceStatus = ""; renderCompanies();
  });
  ...
  document.getElementById("invoiceFunctionSelected").addEventListener("change", (event) => { document.getElementById("enableInvoiceNextBtn").disabled = companyState.enableBlocked || !event.target.checked; });
  ...
  document.getElementById("enableInvoiceNextBtn").addEventListener("click", () => { if (!companyState.enableBlocked && document.getElementById("invoiceFunctionSelected").checked) setEnableInvoiceStep(2); });
  ...
  document.getElementById("backFromCompanyDetailBtn").addEventListener("click", returnFromCompanyDetail);
  ...
  document.getElementById("companyInvoiceFunctionCard").addEventListener("click", () => {
    ...
    if (company.invoiceStatus === "Unopened" || company.invoiceStatus === "Failed") openEnableInvoice(company.id);
      ...
      const taxpayer = ensureCompanyTaxpayer(company);
      openTaxNoDetail(taxpayer.id, "companyDetailView");
  ...
  document.getElementById("resetBatchRecordsBtn").addEventListener("click", () => { ["batchRecordTypeFilter", "batchRecordStatusFilter"].forEach((id) => { document.getElementById(id).value = ""; }); renderBatchRecords(); });
  ...
  document.body.addEventListener("click", (event) => {
    ...
    if (companyDetail) openCompanyDetail(companyDetail.dataset.companyDetail, companyDetail.dataset.companyDetailSource || companyState.detailSource || "customer");
